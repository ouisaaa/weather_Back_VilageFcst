...
//        System.out.println(weatherService.httpResultJSON(weatherService.searchLocation(city, district, neighborhood)));
        log.info("주소: \""+city+" "+district+" "+neighborhood+" \" 의 단기예보 조회 요청");

        return ResponseEntity.ok().body(weatherService.httpResultJSON(weatherService.searchLocation(city, district, neighborhood),nowDate,nowTime).toString());
    }
